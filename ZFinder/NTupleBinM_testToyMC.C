...
void GetBinM(vector<RooUnfoldResponse*> &BinM, vector<TH1D*> &h_gen, vector<TH1D*> &h_reco, bool madgraph=1, int elec=0){
  ...
  for (int i=0; i<t->GetEntries();i++){
    ...
    if (phistar_true<0)continue;
    if (phistar_true>10)phistar_true=9.7;
    if (phistar>10)phistar=9.7;
    double weight =1;
   ...
   if (!madgraph){
    ...
    if (N_MC==5000)mark.DrawLatex(0.19,0.76,"5000 events");
    if (N_MC==50000)mark.DrawLatex(0.19,0.76,"50000 events");
    if (N_MC==-1)mark.DrawLatex(0.19,0.76,"395561 events");
  }
...
void NTupleBinM_testToyMC(){
    vector<RooUnfoldResponse *> m_BinM;
    vector<TH1D *> m_gen, m_reco;
    GetBinM(m_BinM, m_gen, m_reco, 1, 0);

    vector<RooUnfoldResponse *>p_BinM;
    vector<TH1D *> p_gen, p_reco;
    GetBinM(p_BinM, p_gen, p_reco, 0, 0);
 
    for (uint i=0; i<nphistar;i++){
      m_gen[0]->SetBinError(i+1,0);
      p_gen[0]->SetBinError(i+1,0);
    }

    TH2* BinMigr=m_BinM[0]->Hresponse();
    // BinMigr->Draw("COLZ"); 
    ...
    int nbins=60;
    if (N_MC==50000) nbins=180;
    if (N_MC==-1) nbins=720;

    TH2D* Toy_MC=new TH2D("Toy_MC","Toy_MC",nphistar,phistarBinsPlot,nbins,0.7,1.3);
    ...
    for (uint j=0; j<nphistar; j++){
      ...
      double x,y;
      g_BinM_PM->GetPoint(j,x,y); 
      double pull=y-1;
      double error_pull=0;
      if (pull>0) {
  	error_pull=g_BinM_PM->GetErrorYlow(j);
  	error_pull=error_pull*error_pull;
  	error_pull+=(b_med-b_min68)*(b_med-b_min68);
  	error_pull=sqrt(error_pull);
      }
      else{
  	error_pull=g_BinM_PM->GetErrorYhigh(j);
  	error_pull=error_pull*error_pull;
  	error_pull+=(b_max68-b_med)*(b_max68-b_med);
  	error_pull=sqrt(error_pull);
      }
    ...
    Pull->Draw();
    mark.DrawLatex(0.19,0.84,"MadGraph");
    if (N_MC==-1) mark.DrawLatex(0.19,0.80,"unfolded using full Powheg sample");
    if (N_MC==5000) mark.DrawLatex(0.19,0.80,"unfolded using 5000 Powheg events");
    if (N_MC==50000) mark.DrawLatex(0.19,0.80,"unfolded using 50000 Powheg events");
