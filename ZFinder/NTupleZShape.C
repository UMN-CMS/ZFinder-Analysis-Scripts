...
const bool DoToys=false;//to make the code run faster for one reason or anouther turns off toys.
const int elec=1;
const int doMG=0;
...
vector<TH2D*> GetEffTMCToys(bool MC=1){
  ...
  vector<TH2D*> EffToys;
  int Ntoys2Holder = DoToys? Ntoys2: 0;//this way if DoToys is false toys not done
  for (int t=0; t<Ntoys2Holder+1; t++){
...
vector<TH2D*> GetEffSFToys(int type=0){
  gErrorIgnoreLevel = kError;
  vector<TH2D*> EffToys;
  int Ntoys2Holder = DoToys? Ntoys2: 0;
  for (int t=0; t<Ntoys2Holder+1; t++){
...
vector<TH1D*> GetToyBg(TH1F* bg_sf_full){
  vector<TH1D*> bg_sf;
  int NtoysHolder = DoToys? Ntoys: 0;
  for (int t=0; t<NtoysHolder+1; t++){
...
TGraphAsymmErrors* CalcTotalSysU_toyvariation(vector<TGraphAsymmErrors*> graph, bool useMedian=0, int part=1, bool eff=0){
  ...
  for (size_t iphistar=0; iphistar<nphistar; iphistar++){
    ...
    phis.push_back(y);
    int nt=DoToys? Ntoys: 0;
    if (eff) nt=DoToys? Ntoys2: 0;
    uint gs=1+(nt*(part-1));
...
void FillTrigEffFluc(int &idx, vector<TH2D*> ToyEffMC, vector<TH2D*> ToyEffData, vector<TH1D*> &h_eff, double weight, double phistar, double E0_pt, double E0_eta, double E1_pt, double E1_eta, bool doBinM, vector<RooUnfoldResponse*> &BinM_eff, double phistar_true=0){
  ...
  for (int t=0; t<Ntoys2Holder; t++){
    ...
    if (fabs((weight/new_weight)-1)>0.5) cout<<"Weird weight/new weight"<<weight/new_weight<<endl;
    if (!doBinM){
      h_eff[idx]->Fill(phistar,new_weight);
    }
    else{
...
int Ntoys2Holder = DoToys? Ntoys2: 0;
  h_eff=EmptyHVec(Ntoys2Holder*5+1);
  ...
  for (int i=0; i<t->GetEntries();i++){
    ...
    if (idx==1&&DoToys) cout<<"idx did not increase when it should have"<<endl;
...
TH1D* GetDataPhiStarPt(double sampleweight,int up=0){
  ...
  if (up>0){pth=pth*1.003; ptl=ptl*1.003;}
  else if (up<0) {pth=pth*0.997; ptl=ptl*0.997;}
  int idx=0;
  for (int i=0; i<t->GetEntries();i++){
    t->GetEntry(i);
...
int Ntoys2Holder = DoToys? Ntoys2: 0;
  ...
  h_cteq=EmptyHVec(nwcteq);
  h_fsr_pileup=EmptyHVec(3);
  BinM_eff=EmptyBinMVec(Ntoys2Holder*5+1);
...
void NTupleZShape(){
  double Lumi = 19712.;
  double ttbar_weight  = 23.64/4246440.;//put for other purposes a value of 1.1 and .9
  //ttbar_weight=ttbar_weight*1.1;
 ...
 int NtoysHolder = DoToys? Ntoys: 0;
  for (size_t idx=0; idx<NtoysHolder+5; idx++){
    ...
    if (idx==NtoysHolder+1 || idx==NtoysHolder+2 || idx==NtoysHolder+3 || idx==NtoysHolder+4){
    ...
    double scaless=-2;
    double scaless=-2;
    if (idx==NtoysHolder+1) {
      scale =-0.8;
    } 
    if (idx==NtoysHolder+2) {
      scale =-1.2;
    } 
    if (idx==NtoysHolder+3) {
      scaless =0;
    } 
    if (idx==NtoysHolder+4) {
      scaless =-4.0;//Was set to -4? right? Assuming wrong for now and setting to -2
    }
    if (idx==NtoysHolder+5) {
      scalett =-.9;
    } 
    if (idx==NtoysHolder+6) {
      scalett =-1.1;//Was set to -4? right? Assuming wrong for now and setting to -2
    } 
  ...
  if (elec==0)textn+="Dressedtt.root";
  if (elec==1)textn+="BornTTroot";
  if (elec==2)textn+="Nakedtt.root";
  ...
  TGraphAsymmErrors* g_syst_norm_fsr    = CalcTotalSysU_fsr(g_data_norm_fsr_pileup[0],g_data_norm_eff[0]);
