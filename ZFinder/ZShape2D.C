...
{
  double IntValue = 0; //this will be used to normlize the graph.
...
void NormalizeGraph(vector<TGraphAsymmErrors*> &graph, bool doNorm = 0) {
  for (size_t i = 0; i < graph.size(); i++) {
    ...
    for (size_t iphistar = 0; iphistar < nphistar - 1; iphistar++) {
      graphtemp->GetPoint(iphistar, x, y);
      xstot += y;
    }
	...
	for (int j = 0; j < ((int) nphistar); j++) {
      ...
      errorl = graphtemp->GetErrorYlow(iphistar);
      errorh = graphtemp->GetErrorYhigh(iphistar);
...
TGraphAsymmErrors * GetDataFinal(vector<TGraphAsymmErrors *> graph, vector<std::string> slist, bool doNorm = 0, bool doLumi = 1) {
  ...
  for (size_t iphistar = 0; iphistar < nphistar; iphistar++) {
    ...
    outputfile << std::setprecision(3) << phistarBins[iphistar] << "-" << phistarBins[iphistar + 1] << " & " << std::setprecision(2) << (h_mc+l_mc)/2. * 100. << " & " << (h_pu+l_pu)/2. * 100. << " & " << (h_bg+l_bg)/2. * 100. << " & " << (h_pt+l_pt)/2. * 100. << " & " << (h_eff+l_eff)/2. * 100.;
