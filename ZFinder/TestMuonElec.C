...
double vec_MuonAbs[34]={4.02214e+06,
			3.9248e+06,
			3.85512e+06,
			3.73242e+06,
			3.62621e+06,
...
void TestMuonElec(){
  ...
  TH1D *h_T_Abs = (TH1D*)f2.Get("h_data");
  for (uint i=0; i<nphistar; i++){
    ...
    error=error/val;
    error=sqrt(error*error-0.026*0.026);
    h_Muon_Abs->SetBinContent(i+1,vec_MuonAbs[i]/1000.);
    h_Muon_Abs->SetBinError(i+1,error*vec_MuonAbs[i]/1000.);
    h_Elec_Abs->SetBinContent(i+1,val);
    h_Elec_Abs->SetBinError(i+1,error*val);
    ...
    Total_Muon_Abs+=dphistar*vec_MuonAbs[i]/1000.;
    Total_Elec_Abs+=dphistar*val;
  }
  TFile f1("Final_Hist_Norm_MG_Dressed.root");
  TH1D *h_T_Norm = (TH1D*)f1.Get("h_data");
  ...
  for (uint i=0; i<nphistar; i++){
    ...
    error=error/val;
    h_Muon_Norm->SetBinContent(i+1,vec_MuonNorm[i]);
    h_Muon_Norm->SetBinError(i+1,error*vec_MuonNorm[i]);
    h_Elec_Norm->SetBinContent(i+1,val);
    h_Elec_Norm->SetBinError(i+1,error*val);
    ...
    double dphistar=phistarBins[i+1]-phistarBins[i];
    Total_Muon+=dphistar*vec_MuonNorm[i];
  ...
  CreateRatio(r_Elec_Norm,h_Elec_Norm,h_Elec_Norm);
  CreateRatio(r_Muon_Norm,h_Muon_Norm,h_Elec_Norm);
