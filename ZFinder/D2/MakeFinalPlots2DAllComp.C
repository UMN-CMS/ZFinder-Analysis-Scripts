...
void OneDYPlot(vector<TGraphAsymmErrors*> g_data, vector<TGraphAsymmErrors*> g_mg, vector<TGraphAsymmErrors*> g_ph, vector<TGraphAsymmErrors*> g_re, vector<TGraphAsymmErrors*> g_ANlo, vector<TGraphAsymmErrors*> g_Pyth8) {
    ...
    g_Data_Y_ratio->SetFillColor(kGray);
    ...
    if (Type == "elec") {
        leg2->AddEntry(g_mg_Y_ratio, " MadGraph + PYTHIA 6 (Z2*)", "P");
        leg2->AddEntry(g_ph_Y_ratio, "POWHEG + PYTHIA 6 (Z2*)", "P");
        leg2->AddEntry(g_re_Y_ratio, "ResBos", "P");
        leg2->AddEntry(g_ANlo_Y_ratio, "aMC@NLO + PYTHIA 8 (CUETP8M1)", "P");
        leg2->AddEntry(g_Pyh8_Y_ratio, "POWHEG + PYTHIA 8 (CUETP8M1)", "P");
    }
...
void PlotFinal(TGraphAsymmErrors* g_data_final, TGraphAsymmErrors* g_mg_final, TGraphAsymmErrors* g_ph_final, TGraphAsymmErrors* g_ratio_phistar, TGraphAsymmErrors* g_ratio_mg_phistar, TGraphAsymmErrors* g_ratio_ph_phistar, bool isPlot2 = 0, TGraphAsymmErrors* g_re_final = 0, TGraphAsymmErrors* g_ratio_re_phistar = 0) {
    ...
    for (uint i = 0; i < ny; i++) {
        ...
        if (doNorm) {
            r_data[i]->GetYaxis()->SetRangeUser(0.77, 1.23);
            if (i == 4)r_data[i]->GetYaxis()->SetRangeUser(0.74, 1.26);
            if (i == 5) {
                r_data[i]->GetYaxis()->SetRangeUser(0.63, 1.37);
                r_data[i]->GetYaxis()->SetNdivisions(505);
            }
        } else {
            r_data[i]->GetYaxis()->SetRangeUser(0.715, 1.295);
    ...
    if (doNorm && Type == "combined") {
        ...
        mark2.DrawLatex(.05, .28, "Theory/Data for 1/#sigma^{fid} #bullet d^{2}#sigma^{fid}/d#phi*d|y|");
    ...
    leg2->AddEntry(r_data[1], "ee + #mu#mu Combined Data", "F");
    leg2->AddEntry(r_ANlo[1], "aMC@NLO + PYTHIA 8 (CUETP8M1)", "P");
    leg2->AddEntry(r_re[1], "ResBos", "P");
    leg2->AddEntry(r_mg[1], "MadGraph + PYTHIA 6 (Z2*)", "P");
    leg2->AddEntry(r_ph[1], "POWHEG + PYTHIA 6 (Z2*)", "P");
    leg2->AddEntry(r_Pyth8[1], "POWHEG + PYTHIA 8 (CUETP8M1)", "P");
...
void MakeYCompPlots() {
    ...
    for (uint i = 0; i < 5; i++) {
        ...
        AllPlots[i][0]->SetFillColor(kGray);
    ...
    mark2.DrawLatex(.12, .7, "POWHEG + PYTHIA8 (CUETP8M1)");
    //mark.DrawLatex(-0.15, 0.25, "|#eta^{l_{0}}| < 2.1,        |#eta^{l_{1}}| < 2.4");
    //mark.DrawLatex(0.15, 0.20, "p_{T}^{l_{0}} > 30 GeV,   p_{T}^{l_{1}} > 20 GeV");
    ...
    FinalPhiRatio->RedrawAxis();
    if (doNorm) {
        FinalPhiRatio->SaveAs("MC2DCompNorm.pdf");
        FinalPhiRatio->SaveAs("MC2DCompNorm.png");
    } else {
        FinalPhiRatio->SaveAs("MC2DCompAbs.pdf");
         FinalPhiRatio->SaveAs("MC2DCompAbs.png");
    }
