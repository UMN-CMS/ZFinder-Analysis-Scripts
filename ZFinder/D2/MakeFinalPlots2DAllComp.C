...
TGraphAsymmErrors* CombinePlots(TGraphAsymmErrors* First, TGraphAsymmErrors* Second){
    TGraphAsymmErrors* Combined = new GraphAsymmErrors(nbins);
    ...
    for (size_t ibin = 0; ibin < nbins; ibin++) {
        ...
        double NewValue;
        double NewError;
        First->GetPoint(ibin,x1,y1);
        Error1=First->GetErrorYlow(ibin);
        Second->GetPoint(ibin,x2,y2);
        Error2=Second->GetErrorYlow(ibin);
        NewError=sqrt(1/(1/Error1/Error1+1/Error2/Error2));
        NewValue=(y1/Error1/Error1+y2/Error2/Error2)*NewError*NewError;
