...
const double EfficiencyETBins[7] = {10, 15, 20, 30, 40, 50, 1000000};
...
std::string File_Data=       "/afs/cern.ch/work/r/ruckstuh/public/Data.root";
std::string File_Data_Pt=    "/afs/cern.ch/work/r/ruckstuh/public/Data_En.root";
//std::string File_Data_Pt=  "/afs/cern.ch/work/r/ruckstuh/public/Data.root";
...
std::string reco_name_en="Combined Single Lowered Threshold Reco";
...
vector<TH2D*> GetEffSFToys(){
  gErrorIgnoreLevel = kError;
  vector<TH2D*> EffToys;
  for (int t=0; t<Ntoys+1; t++){
    TH2D *h_eff= new TH2D("h_eff","h_eff",5,EfficiencyEtaBins,6,EfficiencyETBins); 
    EffToys.push_back(h_eff);
  }
  for (int e=0; e<5; e++){
    for (int p=0; p<6; p++){
      double mean=EfficiencySF[e][p][0];
      double error= sqrt(pow(EfficiencySF[e][p][1]/EfficiencySF[e][p][0],2.)+pow(EfficiencySF[e][p][2]/EfficiencySF[e][p][0],2.))*mean;
      EffToys[0]->SetBinContent(e+1, p+1, mean);
      for (int t=0; t<Ntoys; t++){
	double x=gRandom->Gaus(mean,error);
	EffToys[t+1]->SetBinContent(e+1, p+1, x);
...
TGraphAsymmErrors* CalcTotalSysU_toyvariation(vector<TGraphAsymmErrors*> graph, bool useMedian=0, int part=1){
  TGraphAsymmErrors* g_un = new TGraphAsymmErrors(nphistar);
  for (size_t iphistar=0; iphistar<nphistar; iphistar++){
    double x,y;
    vector<double> phis;
    ...
    phis.push_back(y);
    uint gs=1+(Ntoys*(part-1));
    for (size_t i=gs; i<gs+Ntoys ;i++){
      graph[i]->GetPoint(iphistar,x,y);
      phis.push_back(y);
    }
    ...
    if (useMedian){
     ...
     double med=phis[(gs-1)/2];
     int idx68=0.1587*(gs)-0.5;
     double min68=phis[idx68];
     double max68=phis[(gs)-idx68];
...
TGraphAsymmErrors* CalcTotalSysU_toymc(vector<TGraphAsymmErrors*> graph, TGraphAsymmErrors* graph_nominal, bool isBG=0, int part=1){
  TGraphAsymmErrors* g_toy=CalcTotalSysU_toyvariation(graph,1,part);
  TGraphAsymmErrors* g_un = new TGraphAsymmErrors(nphistar);
  for (size_t iphistar=0; iphistar<nphistar; iphistar++){
    double x,y,xmc,ymc;
    graph_nominal->GetPoint(iphistar,x,y);
...
TGraphAsymmErrors* GetDataFinal(vector<TGraphAsymmErrors *> graph, vector<std::string> slist, bool doLumi=1){
  ...
  outputfile << "& MC stat. & Bkg. & Eff. & E scale";
  ...
  for (size_t iphistar=0; iphistar<nphistar; iphistar++){
    ...
    for (uint i=0; i<nun; i++){
      ...
      if (slist[i]=="pt")        {h_pt=error_max;l_pt=error_min;}
    ...
    outputfile <<h_mc*100.<<"\\% & "<<h_bg*100.<<"\\% & "<<h_eff*100.<<"\\% & "<<h_pt*100.<<"\\% & ";
...
void FillRecoEffFluc(int &idx, vector<TH2D*> ToyEff, vector<TH1D*> &h_eff, double weight, double phistar, double E0_pt, double E0_eta, double E1_pt, double E1_eta, bool doBinM, vector<RooUnfoldResponse*> BinM_eff, double phistar_true=0){
...
void GetBGPhiStar(std::string FileName, double sampleweight, vector<TH1D*> &h_eff, vector<TH1D*> &h_fsr_pileup, vector<TH2D*> h_ToyEffSF, vector<TH2D*> h_ToyEffTMC, vector<TH2D*> h_ToyEffTData){
  gErrorIgnoreLevel = kError;
...
TH1D* GetDataPhiStar(double sampleweight){
  cout<<"reading data"<<endl;
  TH1D *h_phistar= new TH1D("phistar","phistar",nphistar,phistarBins);
  h_phistar->Sumw2();
  TChain* t = new TChain(reco_name.c_str(),reco_name.c_str());
  int nfiles;
  nfiles=t->Add(File_Data.c_str());
  TBranch *b_reco=t->GetBranch("reco");
  TLeaf *l_phistar=b_reco->GetLeaf("z_phistar_dressed");
  cout<<"Entries: "<<t->GetEntries()<<endl;
  for (int i=0; i<t->GetEntries();i++){
...
TH1D* GetDataPhiStarPt(double sampleweight,bool up=1){
  cout<<"reading data"<<endl;
  TH1D *h_phistar= new TH1D("phistar","phistar",nphistar,phistarBins);
  h_phistar->Sumw2();
  std::string name=reco_name;
  if (!up) name=reco_name_en;
  TChain* t = new TChain(name.c_str(),name.c_str());
  int nfiles;
  if (up) nfiles=t->Add(File_Data.c_str());
  else nfiles=t->Add(File_Data_Pt.c_str());
  TBranch *b_reco=t->GetBranch("reco");
  TLeaf *l_phistar=b_reco->GetLeaf("z_phistar_dressed");
  ...
  if (up){pth=pth*1.003; ptl=ptl*1.003;}
  else {pth=pth*0.997; ptl=ptl*0.997;}
  int idx=0;
  for (int i=0; i<t->GetEntries();i++){
    t->GetEntry(i);
    ...
    if ((pt0 < pth || fabs(eta0) > etah) && (pt1 < pth || fabs(eta1) > etah)) continue; 
    h_phistar->Fill(l_phistar->GetValue(),sampleweight);
    idx++;
  }
...
void GetBinM(double sampleweight, vector<RooUnfoldResponse*> &BinM_eff, vector<RooUnfoldResponse*> &BinM_mcstat, vector<RooUnfoldResponse*> &BinM_cteq, vector<RooUnfoldResponse*> &BinM_fsr_pileup, vector<TH1D*> &h_eff, vector<TH1D*> &h_cteq, vector<TH1D*> &h_fsr_pileup, vector<TH2D*> h_ToyEffSF, vector<TH2D*> h_ToyEffTMC, vector<TH2D*> h_ToyEffTData){
  gErrorIgnoreLevel = kError;
  ...
  h_cteq=EmptyHVec(nwcteq);
  h_fsr_pileup=EmptyHVec(3);
  BinM_eff=EmptyBinMVec(Ntoys*3+1);
...
void NTupleZShape(){
  ...
  mc_truegen->Divide(mc_truegen_fsr_pileup[0]);
  mc_truegen->Draw();
  ...
  GetBinM(signal_weight, BinM_eff, BinM_mcstat, BinM_cteq, BinM_fsr_pileup, mc_truereco_eff, mc_truereco_cteq, mc_truereco_fsr_pileup, h_ToyEffSF, h_ToyEffTMC, h_ToyEffTData);

  TH1D* Data_down=GetDataPhiStarPt(1./Lumi,0);
 ...
 vector<TH1D*> h_tt_eff,     h_tt_fsr_pileup;
  ...
  GetBGPhiStar(File_tt,    ttbar_weight, h_tt_eff,     h_tt_fsr_pileup,     h_ToyEffSF, h_ToyEffTMC, h_ToyEffTData);
  GetBGPhiStar(File_tautau,tautau_weight,h_tautau_eff, h_tautau_fsr_pileup, h_ToyEffSF, h_ToyEffTMC, h_ToyEffTData);
  GetBGPhiStar(File_tbarw, tbarw_weight, h_tbarw_eff,  h_tbarw_fsr_pileup,  h_ToyEffSF, h_ToyEffTMC, h_ToyEffTData);
  GetBGPhiStar(File_tw,    tw_weight,    h_tw_eff,     h_tw_fsr_pileup,     h_ToyEffSF, h_ToyEffTMC, h_ToyEffTData);
  GetBGPhiStar(File_ww,    ww_weight,    h_ww_eff,     h_ww_fsr_pileup,     h_ToyEffSF, h_ToyEffTMC, h_ToyEffTData);
  GetBGPhiStar(File_wz,    wz_weight,    h_wz_eff,     h_wz_fsr_pileup,     h_ToyEffSF, h_ToyEffTMC, h_ToyEffTData);
  GetBGPhiStar(File_zz,    zz_weight,    h_zz_eff,     h_zz_fsr_pileup,     h_ToyEffSF, h_ToyEffTMC, h_ToyEffTData);
  ...
  TGraphAsymmErrors* g_syst_phistar_reff   = CalcTotalSysU_toymc(g_data_phistar_eff,g_data_phistar_eff[0],0,1);
  TGraphAsymmErrors* g_syst_phistar_teff_m = CalcTotalSysU_toymc(g_data_phistar_eff,g_data_phistar_eff[0],0,2);
  TGraphAsymmErrors* g_syst_phistar_teff_d = CalcTotalSysU_toymc(g_data_phistar_eff,g_data_phistar_eff[0],0,3);
  TGraphAsymmErrors* g_syst_phistar_eff    = CalcTotalSysU_comb3(g_syst_phistar_reff,g_syst_phistar_teff_m,g_syst_phistar_teff_d,1);
